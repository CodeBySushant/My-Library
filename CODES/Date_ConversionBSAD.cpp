//Completed but have some issue which need to be fixed
#include<iostream>
using namespace std;
...
const int bs_month_days[] = {30, 32, 31, 32, 32, 30, 30, 30, 29, 30, 29, 30}; //Days of Every Month in Year using Arrays
...
void AdToBs(int ad_year, int ad_month, int ad_day){
    ...
    int bs_day = REF_BS_DAY;

    for(int i=REF_AD_MONTH; i<ad_month; ++i){
        bs_day = bs_day + 30; // Adding days for each AD month
        if(bs_day > bs_month_days[bs_month - 1]){
            bs_month++;
            bs_day = bs_day - bs_month_days[bs_month - 2];  // Adjust the day within the BS month
        }
    }
    bs_day = bs_day + (ad_day - REF_AD_DAY);
    while(bs_day > bs_month_days[bs_month - 1]){
        bs_month++;
        bs_day = bs_day - bs_month_days[bs_month - 1];  // Adjust day if it exceeds the days in a month
    }
    while(bs_month > 12){
        bs_year++;
        bs_month = bs_month - 12;  // Move to the next year if month exceeds 12
    }
